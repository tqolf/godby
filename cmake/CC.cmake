...
function (cc_enable_feature_for_target target)
...
function (cc_target type)
  ...
  set(multipleOptions HEADERS SOURCES OPTIONS LINK_OPTIONS INCLUDES DEPENDENCIES FEATURES)
  ...
  cc_enable_feature_for_target(${cc_NAME})
  if (cc_FEATURES)
    foreach (check IN LISTS cc_FEATURES)
      string(TOUPPER "${check}" CHECK)
      set(FEATURES_${CHECK} ON FORCE)
    endforeach ()
  endif ()
  ...
  get_target_property(aliased ${cc_NAME} ALIASED_TARGET)
  if (aliased STREQUAL "${cc_NAME}-NOTFOUND")
    target_link_options(${cc_NAME} PRIVATE $<$<COMPILE_LANG:C,CXX>:-Wl,-z,noexecstack>)
  endif ()
  ...
  foreach (depend ${cc_DEPENDENCIES})
    ...
    if ("${link_behavior}" STREQUAL "WHOLE_ARCHIVE")
      ...
      foreach (target IN LISTS ALL_TARGETS)
        if (APPLE)
          target_link_libraries(${target} PUBLIC -Wl,-force_load ${depend})
        elseif (MSVC)
          # In MSVC, we will add whole archive in default.
          target_link_libraries(${target} PUBLIC -WHOLEARCHIVE:${depend})
        else ()
          # Assume everything else is like gcc
          target_link_libraries(${target} PUBLIC -Wl,--allow-multiple-definition)
          target_link_libraries(${target} PUBLIC -Wl,--whole-archive ${depend} -Wl,--no-whole-archive)
        endif ()
      endforeach ()
    else ()
      foreach (target IN LISTS ALL_TARGETS)
