...
int32_t allocate(int8_t threads, int32_t limit)
{
	...
	for (int8_t i = 0; i < threads; i++) {
		std::thread *tid = new std::thread([&]() {
			...
			while (1) {
				...
				for (int16_t i = 0; i < limit; i++) { pool.delete_element(nodes[i]); }

				nodes.clear();
				if ((++counter % 10) == 0) { fprintf(stdout, "Counter: %ld\n", counter.load()); }
			}
		});
