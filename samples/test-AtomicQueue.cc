...
struct test_stateful_allocator : std::allocator<T> {
	...
	test_stateful_allocator(std::nullptr_t, const State &s) noexcept : state(s) {}

	test_stateful_allocator(const test_stateful_allocator &other) noexcept : std::allocator<T>(other), state(other.state) {}

	template <class U>
	test_stateful_allocator(const test_stateful_allocator<U, State> &other) noexcept : state(other.state)
	{
	}
...
{
	using Queue = godby::AtomicQueueB2<
		/* T = */ float,
	...
	for (unsigned i = 1; i <= CAPACITY; ++i) { BOOST_CHECK(q.try_push(i)); }
...
{
	godby::RetryDecorator<godby::AtomicQueueB2<float>> q(10);
	BOOST_CHECK_EQUAL(q.capacity(), CACHE_LINE_SIZE * CACHE_LINE_SIZE);
}

BOOST_AUTO_TEST_CASE(power_of_2)
{
	using godby::NextPowerOfTwo;
	static_assert(NextPowerOfTwo(0u) == 1u, "");
	static_assert(NextPowerOfTwo(1u) == 1u, "");
	static_assert(NextPowerOfTwo(2u) == 2u, "");
	static_assert(NextPowerOfTwo(3u) == 4u, "");
	static_assert(NextPowerOfTwo(127u) == 128u, "");
	static_assert(NextPowerOfTwo(128u) == 128u, "");
	static_assert(NextPowerOfTwo(129u) == 256u, "");
	static_assert(NextPowerOfTwo(0x40000000u - 1) == 0x40000000u, "");
	static_assert(NextPowerOfTwo(0x40000000u) == 0x40000000u, "");
	static_assert(NextPowerOfTwo(0x40000000u + 1) == 0x80000000u, "");
}
