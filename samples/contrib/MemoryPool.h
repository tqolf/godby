...
class MemoryPool {
  public:
	...
	{
		return m_max_size * sizeof(slot_t);
	}
	std::size_t max_number_objects() const noexcept
	{
		return m_max_size;
	}
	...
	void set_allocate_block_threshold(uint32_t thresh)
	{
		m_allocate_block_threshold = thresh;
	}
...
inline typename MemoryPool<T, block_size>::size_type MemoryPool<T, block_size>::pad_pointer(data_pointer p, size_type align) const noexcept
{
...
MemoryPool<T, block_size>::MemoryPool(MemoryPool &&mp) noexcept
	: m_max_size(mp.m_max_size), m_last_slot(nullptr), m_free(mp.m_free), m_lock(mp.m_lock), m_allocated_block_head(nullptr)
{
...
template <typename T, std::size_t block_size>
MemoryPool<T, block_size> &MemoryPool<T, block_size>::operator=(MemoryPool &&mp)
{
...
inline typename MemoryPool<T, block_size>::pointer MemoryPool<T, block_size>::allocate(size_type n, const_pointer hint)
{
	...
	} while (!m_free.compare_exchange_weak(orig, next));
...
template <typename T, std::size_t block_size>
template <class U>
inline void MemoryPool<T, block_size>::destroy(U *p)
{
	if (p != nullptr) { p->~U(); }
}
...
template <typename T, std::size_t block_size>
template <class... Args>
inline typename MemoryPool<T, block_size>::pointer MemoryPool<T, block_size>::new_element(Args &&...args)
{
...
template <typename T, std::size_t block_size>
inline void MemoryPool<T, block_size>::delete_element(pointer p)
{
	if (p != nullptr) {
		p->~value_type();
		deallocate(p);
...
template <typename T, std::size_t block_size>
inline bool MemoryPool<T, block_size>::allocate_block()
{
	...
	if (m_max_size > 0 && (now <= m_last_allocate_block_time + std::chrono::seconds(m_allocate_block_threshold))) { return false; }
	...
	if (m_last_slot) { m_last_slot->next = reinterpret_cast<slot_t *>(start); }
	...
	if (m_free.load().node != nullptr) { m_last_slot->next = m_free.load().node; }
